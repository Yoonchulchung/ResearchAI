...
export class JobplanetScraperService {
  ...
  async testLogin(
    ...
    try {
      ...
      const { ok, reused, finalUrl, error, failedStep } =
        await this.jobplanetAuth.loginWithSession(page, id, password, onLog);
      if (!ok) {
        onLog?.(
          `로그인 실패: ${failedStep ?? '알 수 없는 단계'}${error ? ` - ${error}` : ''}`,
        );
        return { success: false, finalUrl, error, failedStep };
      }
      onLog?.(
        `로그인 성공${reused ? ' (저장된 세션 사용)' : ''}: ${finalUrl ?? page.url()}`,
      );
      return {
        success: true,
      ...
      onLog?.(
        `브라우저 실행 또는 로그인 테스트 오류: ${(err as Error).message}`,
      );
      return {
        success: false,
        failedStep: '브라우저 실행',
  ...
  async debugPage(
    ...
    try {
      ...
      const companyLink =
        foundLinks.find((l) => l.includes('/companies/')) ?? null;
      const reviewUrl = companyLink
        ? companyLink.replace(/\/$/, '') + '/reviews'
        : searchUrl;
  ...
  async findOfficialWebsite(companyName: string): Promise<string | null> {
    ...
    try {
      ...
      const firstResult = await page.evaluate(
        (exclude: string[], name: string) => {
          const norm = (s: string) =>
            s.replace(/[\s(주)㈜()]/g, '').toLowerCase();
          const links = Array.from(
            document.querySelectorAll(
              'a.link_tit[href^="http"], a.total_tit[href^="http"], .api_subject_bx a[href^="http"], .source_box a[href^="http"]',
            ),
          );
      ...
      this.logger.warn(
        `[Scraper] 공식 웹사이트 탐색 오류: ${(err as Error).message}`,
      );
      return null;
    } finally {
      await browser?.close();
  ...
  async scrapeCompany(
    ...
    try {
      ...
      const { ok, finalUrl, failedStep, error } =
        await this.jobplanetAuth.loginWithSession(page, id, password, onLog);
      if (!ok) {
        this.logger.warn('[Jobplanet] 로그인 실패 — 스크래핑 중단');
        onLog?.(
          `로그인 실패로 수집 중단: ${failedStep ?? '로그인'}${error ? ` - ${error}` : ''}`,
        );
        return null;
      }
  ...
  private toReviewUrl(href: string): string {
    ...
    return href
      .replace(/\/companies\/(\d+)\/[^/]+\/(.+)/, '/companies/$1/reviews/$2')
      .replace(/\/companies\/(\d+)(?:\/[^/]*)?$/, '/companies/$1/reviews');
  }

  private async scrapeCompanyData(
    page: Page,
    companyName: string,
    onLog?: JobplanetLogFn,
  ): Promise<JobplanetCompanyData | null> {
    try {
      ...
      const data = await page.evaluate((name) => {
        // ── 전체 텍스트 ───────────────────────────────────────────
        const rawSummary = (document.body.innerText ?? '')
          .replace(/[ \t]+/g, ' ')
          .trim()
          .slice(0, 80000);
        ...
        // ── 노이즈 섹션 경계 컷오프 ──────────────────────────────────
        // 잡플래닛 페이지에서 실제 기업 리뷰와 무관한 섹션들의 시작 키워드
        const NOISE_MARKERS = [
          '이 리뷰와 비슷한 다른 기업 리뷰',
          '비슷한 기업 리뷰',
          '이런 기업은 어때요',
        ...
        for (const marker of NOISE_MARKERS) {
          const idx = filteredBody.indexOf(marker);
          if (idx > 300) filteredBody = filteredBody.slice(0, idx);
        }

        // ── 종합 평점 ─────────────────────────────────────────────
        // "전체 리뷰 통계 (N명)\n3.8" 패턴
        const overallM = body.match(
          /통계\s*\(\d[\d,]*명\)\s*\n?\s*([1-5]\.[0-9])/,
        );
        ...
        //   경영진에 바라는 점
        //   [내용]
        const reviewBlocks = filteredBody
          .split(/(?=\d{4}\.\s*\d{2}\s*작성)/)
          .slice(1, 31);
        ...
        return {
          ...
          wlbRating,
          reviews,
          // 리뷰 파싱 실패 시 노이즈 제거된 기업 본문만 AI에 전달
          rawSummary: reviews.length === 0 ? filteredBody.slice(0, 15000) : '',
        };
      }, companyName);
      ...
      this.logger.error(
        `[Jobplanet] 데이터 추출 오류: ${(err as Error).message}`,
      );
    ...
    try {
      ...
      const companyLink = await page.evaluate((query) => {
        ...
        const anchors = Array.from(
          ...
          /\/companies\/\d+/.test((el as HTMLAnchorElement).href),
        );
        const match =
          anchors.find((a) => {
            const t = norm(a.textContent?.trim() ?? '');
