...
import { YearlyFinancial } from 'src/financial/infrastructure/dart/dart-types';
...
@Injectable()
export class CompanyAnalysisImplService implements OnModuleInit {
  private readonly logger = new Logger(CompanyAnalysisImplService.name);
  ...
  async buildChatContext(companyKey: string): Promise<string> {
    ...
    if (!row) throw new NotFoundException(`기업 분석 결과 없음: ${companyKey}`);
    const rate = await this.rateRepo.findOne({ where: { companyKey } });
    return this.chat.formatChatContext(
      toDto(row, rate ?? null),
      row.sourceContext,
    );
  }
  ...
  private async enrichDtoWithNaverFinancials(
    ...
    try {
      ...
      this.logger.warn(
        `[Naver Finance Scraper] 오류: ${(err as Error).message}`,
      );
    }
  }
}
