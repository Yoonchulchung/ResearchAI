...
@Injectable()
export class CompanyNewsImplService {
  private readonly logger = new Logger(CompanyNewsImplService.name);
  ...
  async fetchAndSaveNews(
    ...
    items = this.filterCompanyTitleItems(items, companyName);
  ...
  async fetchLatestNewsSinceLastCollection(
    ...
    // Google 병렬 수집은 CAPTCHA로 차단됨 — 추후 Serper API 키 확보 시 주석 해제
    // const [naverRaw, googleRaw] = await Promise.allSettled([...]);
    if (this.getNaverCredentials()) {
      for (let start = 1; start < 1000; start += pageSize) {
        const page = await this.callNaverApi(
          `"${companyName}" 뉴스`,
          pageSize,
          start,
        );
        raw.push(...page);
        if (page.length < pageSize || !hasValidLatestDate) break;
        const oldestPublishedAt = page.reduce<Date | null>((oldest, item) => {
        ...
        if (oldestPublishedAt && oldestPublishedAt <= latestPublishedAt) break;
      }
    } else {
    ...
    const deduped = this.filterCompanyTitleItems(
      deduplicateNewsItems(this.dedupeNewsItems(inRequestedRange)),
      companyName,
    ...
    await this.saveItems(companyId, enriched);
    return enriched;
  }
  ...
  async resetSavedNews(companyId: string): Promise<{
    deletedNews: number;
    deletedKeywords: number;
    deletedTimeline: number;
  }> {
    const [newsResult, keywordResult, timelineResult] = await Promise.all([
      this.newsRepo.delete({ companyId }),
      this.keywordRepo.delete({ companyId }),
      this.timelineRepo.delete({ companyId }),
    ]);

    return {
      deletedNews: newsResult.affected ?? 0,
      deletedKeywords: keywordResult.affected ?? 0,
      deletedTimeline: timelineResult.affected ?? 0,
    };
  }
  ...
  private async collectHybridNews(
    ...
    const [naverResult, googleResult] = await Promise.allSettled([
      ...
      this.searchGoogleNews(companyName, searchLimit, searchOffset),
    ]);
    ...
    if (googleResult.status === 'rejected') {
      this.logger.warn(
        `Google 뉴스 검색 실패 (${companyName}): ${this.errorMessage(googleResult.reason)}`,
      );
    }

    return this.filterCompanyTitleItems(
      deduplicateNewsItems(
        this.dedupeNewsItems([...naverItems, ...googleItems]),
      ),
      companyName,
    ).filter((item) => !isInvestmentNews(item.title, item.snippet ?? ''));
  }
  ...
  async bulkFetchAndSaveNews(
    ...
    const deduped = this.filterCompanyTitleItems(
      deduplicateNewsItems(this.dedupeNewsItems(raw)),
      companyName,
    ).filter((item) => !isInvestmentNews(item.title, item.snippet ?? ''));
  ...
  private filterCompanyTitleItems(
    items: CompanyNewsItem[],
    companyName: string,
  ): CompanyNewsItem[] {
    return items.filter((item) =>
      newsTitleIncludesCompanyName(item.title, companyName),
    );
  }
  ...
  private async searchGoogleNews(
    ...
    const results = await this.browser.searchGoogleNews(query, limit + offset);
