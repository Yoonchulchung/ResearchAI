...
import { JobplanetScraperService, JobplanetCompanyData } from '../infrastructure/jobplanet-scraper.service';
...
export class CompanyAnalysisService implements OnModuleInit {
  ...
  // jobplanet_summary 컬럼이 company_analyses에 남아있는 경우 company_rate로 이전
  private async migrateJobplanetSummaryToRate() {
    try {
      ...
      ) as { company_key: string; company_name: string; jobplanet_summary: string }[];

      if (!rows.length) return;

      for (const row of rows) {
        const exists = await this.rateRepo.findOne({ where: { companyKey: row.company_key } });
        if (!exists) {
      ...
      this.logger.log(`[CompanyRate] 잡플래닛 요약 ${rows.length}건 마이그레이션 완료`);
    } catch {
  ...
  async findAll(): Promise<CompanyAnalysisDto[]> {
    const rows = await this.repo.find({ order: { updatedAt: 'DESC' }, relations: ['company', 'company.financial'] });
    const keys = rows.map((r) => r.companyKey);
    const rates = keys.length
      ? await this.rateRepo.find({ where: keys.map((k) => ({ companyKey: k })) })
      : [];
    const rateMap = new Map(rates.map((r) => [r.companyKey, r]));
    return rows.map((r) => this.toDto(r, rateMap.get(r.companyKey) ?? null));
  }

  async findByKey(companyKey: string): Promise<CompanyAnalysisDto> {
    const row = await this.repo.findOne({ where: { companyKey }, relations: ['company', 'company.financial'] });
    if (!row) throw new NotFoundException(`기업 분석 결과 없음: ${companyKey}`);
    const rate = await this.rateRepo.findOne({ where: { companyKey } });
    return this.toDto(row, rate ?? null);
  }

  async findByName(companyName: string): Promise<CompanyAnalysisDto | null> {
    const key = this.normalizeKey(companyName);
    const row = await this.repo.findOne({ where: { companyKey: key }, relations: ['company', 'company.financial'] });
    if (!row) return null;
  ...
  async *analyzeStream(companyName: string, aiModel: string, signal?: AbortSignal): AsyncGenerator<CompanyAnalysisProgress> {
    const key = this.normalizeKey(companyName);
    ...
    if (creds.jobplanetId && creds.jobplanetPassword) {
      ...
      try {
        jpData = await this.jobplanetScraper.scrapeCompany(companyName, creds.jobplanetId, creds.jobplanetPassword);
        if (jpData) {
    ...
    // company 엔티티에 분석 과정에서 얻은 추가 정보 반영
    if (company) {
    ...
    const rateEntity = jpData ? await this.upsertCompanyRate(key, companyName, jpData, jobplanetText) : await this.rateRepo.findOne({ where: { companyKey: key } });
    // toDto에서 company/financial 관계 사용 — 저장 후 reload
    const entityWithCompany = await this.repo.findOne({ where: { id: entity.id }, relations: ['company', 'company.financial'] }) ?? entity;
    yield { type: 'done', result: this.toDto(entityWithCompany, rateEntity ?? null) };
  }
  ...
  async buildChatContext(companyKey: string): Promise<string> {
    ...
    if (!row) throw new NotFoundException(`기업 분석 결과 없음: ${companyKey}`);
    const rate = await this.rateRepo.findOne({ where: { companyKey } });
    return this.formatChatContext(this.toDto(row, rate ?? null), row.sourceContext);
  }
  ...
  private toDto(e: CompanyAnalysisEntity, rate?: CompanyRateEntity | null): CompanyAnalysisDto {
    const parse = <T>(json: string | null): T | null => {
      if (!json) return null;
      try { return JSON.parse(json) as T; } catch { return null; }
    };
    const c = e.company ?? null; // companies 관계 (로드된 경우)
    const fin = c?.financial ?? null; // company_financial 관계
    ...
    return {
      ...
      employeeHistory: (() => {
        const raw = parse<EmployeeDetail | EmployeeDetail[]>(fin?.employeeDetail ?? null);
        if (!raw) return null;
      ...
      multiYearFinancials: parse<YearlyFinancial[]>(fin?.multiYearFinancials ?? null),
      financialSummary: fin?.financialSummary ?? null,
      disclosures: parse<{ title: string; date: string; url: string }[]>(fin?.disclosures ?? null),
