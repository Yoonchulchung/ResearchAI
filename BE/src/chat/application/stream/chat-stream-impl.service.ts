...
import { ChatRole } from 'src/chat/domain/chat-message.model';
...
import { ChatStreamEvent } from 'src/chat/application/chat.service';

@Injectable()
export class ChatStreamImplService {
...
확실하지 않으면 호출하지 마세요.`;
    ...
    ).slice(-ChatStreamImplService.WINDOW_SIZE);
  ...
  private async getHistoryForAI(sessionId: string) {
    const rows = await this.chatRepository.findBySessionId(sessionId);
    return rows.map((row) => ({
      role: row.whoSent === WhoSent.USER ? ChatRole.USER : ChatRole.ASSISTANT,
