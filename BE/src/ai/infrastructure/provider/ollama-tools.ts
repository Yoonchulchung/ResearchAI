 ...
 *
 * 사용 예:
 *   import { BASIC_TOOLS, executeOllamaTool } from './ollama-tools';
 *
...
import type { OllamaTool } from './ollama.ai';
...
export const TOOL_GET_CURRENT_TIME: OllamaTool = {
  ...
  function: {
    ...
    description: '현재 날짜와 시각을 반환합니다. 오늘이 몇 월 며칠인지, 현재 시간이 몇 시인지 알 때 사용합니다.',
    parameters: {
      type: 'object',
      properties: {
        timezone: {
          type: 'string',
          description: 'IANA 타임존 (예: "Asia/Seoul", "UTC"). 기본값은 Asia/Seoul.',
        },
        format: {
          type: 'string',
          description: '출력 형식: "datetime" | "date" | "time" | "iso". 기본값은 "datetime".',
...
export const TOOL_SEARCH_WEB: OllamaTool = {
  ...
  function: {
    name: 'search_web',
    description: '웹 검색을 수행하고 관련 정보를 반환합니다. 최신 정보, 뉴스, 기술 문서, 회사 정보 등을 찾을 때 사용합니다.',
    parameters: {
      type: 'object',
      properties: {
        query: {
...
export const TOOL_FETCH_URL: OllamaTool = {
  ...
  function: {
    ...
    description: '특정 URL의 웹 페이지 내용을 가져옵니다. 공식 문서, 블로그 글, 뉴스 기사 등 특정 페이지의 내용을 읽어야 할 때 사용합니다.',
    parameters: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description: '내용을 가져올 URL (http:// 또는 https://로 시작해야 함)',
        },
        max_chars: {
          type: 'number',
...
export const TOOL_CALCULATE: OllamaTool = {
  ...
  function: {
    name: 'calculate',
    description: '수학 계산식을 계산합니다. 사칙연산, 거듭제곱, 나머지 연산, 괄호 등을 지원합니다.',
    parameters: {
      type: 'object',
      properties: {
        expression: {
          type: 'string',
          description: '계산할 수식 (예: "2 + 3 * 4", "(100 / 5) ** 2", "17 % 3")',
        },
      },
      required: ['expression'],
    },
...
export const TOOL_CONVERT_UNIT: OllamaTool = {
  ...
  function: {
    name: 'convert_unit',
    description: '단위를 변환합니다. 길이, 무게, 온도, 화폐(환율 제외) 등의 변환에 사용합니다.',
    parameters: {
      type: 'object',
      properties: {
        value: {
...
export async function executeOllamaTool(
  ...
  try {
    ...
    return { success: false, output: `도구 실행 오류 (${name}): ${err?.message ?? err}` };
  }
}
...
async function searchViaTavily(query: string, maxResults: number, apiKey: string): Promise<string> {
...
async function searchViaSerper(query: string, maxResults: number, apiKey: string): Promise<string> {
...
async function searchViaBrave(query: string, maxResults: number, apiKey: string): Promise<string> {
...
async function execFetchUrl(args: Record<string, any>): Promise<string> {
  ...
  if (!url?.startsWith('http')) return 'URL은 http:// 또는 https://로 시작해야 합니다.';
...
const UNIT_CONVERSIONS: Record<string, Record<string, (v: number) => number>> = {
  // 길이 → 미터 기준
  km:   { m: (v) => v * 1000,      cm: (v) => v * 100_000, mm: (v) => v * 1_000_000, mile: (v) => v * 0.621371, yard: (v) => v * 1093.61, feet: (v) => v * 3280.84, inch: (v) => v * 39370.1 },
  m:    { km: (v) => v / 1000,     cm: (v) => v * 100,     mm: (v) => v * 1000,       mile: (v) => v * 0.000621371, feet: (v) => v * 3.28084, inch: (v) => v * 39.3701 },
  cm:   { m: (v) => v / 100,       km: (v) => v / 100_000, mm: (v) => v * 10,         inch: (v) => v * 0.393701, feet: (v) => v * 0.0328084 },
  mm:   { cm: (v) => v / 10,       m: (v) => v / 1000,     km: (v) => v / 1_000_000, inch: (v) => v * 0.0393701 },
  inch: { cm: (v) => v * 2.54,     m: (v) => v * 0.0254,   feet: (v) => v / 12,       mm: (v) => v * 25.4 },
  feet: { m: (v) => v * 0.3048,    cm: (v) => v * 30.48,   inch: (v) => v * 12,       km: (v) => v * 0.0003048 },
  mile: { km: (v) => v * 1.60934,  m: (v) => v * 1609.34,  feet: (v) => v * 5280 },
  yard: { m: (v) => v * 0.9144,    feet: (v) => v * 3,     inch: (v) => v * 36 },
  // 무게 → kg 기준
  kg:   { g: (v) => v * 1000, mg: (v) => v * 1_000_000, lb: (v) => v * 2.20462, oz: (v) => v * 35.274, t: (v) => v / 1000 },
  g:    { kg: (v) => v / 1000, mg: (v) => v * 1000,      lb: (v) => v * 0.00220462, oz: (v) => v * 0.035274 },
  ...
  // 데이터 크기
  byte: { kb: (v) => v / 1024, mb: (v) => v / 1024 ** 2, gb: (v) => v / 1024 ** 3, tb: (v) => v / 1024 ** 4 },
  kb:   { byte: (v) => v * 1024, mb: (v) => v / 1024, gb: (v) => v / 1024 ** 2 },
  mb:   { byte: (v) => v * 1024 ** 2, kb: (v) => v * 1024, gb: (v) => v / 1024, tb: (v) => v / 1024 ** 2 },
  gb:   { byte: (v) => v * 1024 ** 3, kb: (v) => v * 1024 ** 2, mb: (v) => v * 1024, tb: (v) => v / 1024 },
  tb:   { gb: (v) => v * 1024, mb: (v) => v * 1024 ** 2, kb: (v) => v * 1024 ** 3 },
};
