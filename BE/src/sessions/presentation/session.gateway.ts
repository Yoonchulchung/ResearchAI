...
export class SessionGateway implements OnGatewayDisconnect {
  ...
  // 데이터 소스별 최신 상태 집계
  private dataSourceStatuses = new Map<
    string,
    { name: string; pending: number; running: number; cacheSize: number }
  >();
  // 기업 수집 큐 상태를 구독하는 클라이언트 집합
  private enrichQueueSubscribers = new Set<WebSocket>();
  private enrichQueueStatus: {
    pending: number;
    processing: boolean;
  ...
  handleSubscribeDataSources(@ConnectedSocket() client: WebSocket): void {
    ...
    if (client.readyState === WebSocket.OPEN) {
      client.send(
        JSON.stringify({ event: 'data-sources:update', data: statuses }),
      );
    }
  }
  ...
  handleSubscribeEnrichQueue(@ConnectedSocket() client: WebSocket): void {
    ...
    if (client.readyState === WebSocket.OPEN) {
      client.send(
        JSON.stringify({
          event: 'enrich-queue:update',
          data: this.enrichQueueStatus,
        }),
      );
    }
  }

  emitEnrichQueueUpdate(status: {
    pending: number;
    processing: boolean;
    currentCompany: string | null;
    estimatedMs: number | null;
