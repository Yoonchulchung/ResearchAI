...
export class CatchAuthService {
  ...
  async loginWithSession(
    ...
    if (result.ok) {
      ...
      onLog?.(
        `캐치 신규 로그인 성공 - 쿠키 ${this.savedCookies.length}개 저장`,
      );
    }
  ...
  private async doFreshLogin(
    ...
    try {
      ...
      onLog?.(
        `캐치 로그인 페이지 로드 완료: title="${diagnostics.title}", url=${diagnostics.url}`,
      );
      ...
      if (apiResult.attempted)
        onLog?.('캐치 API 로그인 실패 - DOM 입력 방식으로 fallback');

      const idOk = await BrowserAutomationUtil.fillInput(
        page,
        [
          '#id_login',
          'input[id="id_login"]',
      ...
      if (!idOk) {
        ...
        onLog?.(
          `캐치 아이디 입력 필드 없음 - input 후보: ${nextDiagnostics.inputs.join(' | ') || '없음'}`,
        );
        return {
          ok: false,
          finalUrl: nextDiagnostics.url,
          failedStep: '아이디 입력',
          error: '캐치 아이디 입력 필드를 찾을 수 없습니다.',
        };
      }
      ...
      if (!pwOk) {
        const nextDiagnostics =
          await BrowserAutomationUtil.getPageDiagnostics(page);
        onLog?.(
          `캐치 비밀번호 입력 필드 없음 - input 후보: ${nextDiagnostics.inputs.join(' | ') || '없음'}`,
        );
        return {
          ok: false,
          finalUrl: nextDiagnostics.url,
          failedStep: '비밀번호 입력',
          error: '캐치 비밀번호 입력 필드를 찾을 수 없습니다.',
        };
      }

      onLog?.('캐치 로그인 버튼 클릭');
      const clicked =
        (await BrowserAutomationUtil.clickTextButton(page, [/^로그인$/])) ||
        (await BrowserAutomationUtil.submitForm(
          page,
          ['a.mem_btn_join', '.mem_btn_join.bg1', 'button[type="submit"]'],
          this.logger,
          'Catch',
        ));
      if (!clicked) {
        return {
          ...
          finalUrl: page.url(),
          failedStep: '로그인 제출',
          error: '캐치 로그인 버튼을 찾을 수 없습니다.',
        };
      }

      await BrowserAutomationUtil.waitBrieflyForNavigation(page, 3_000);
      await page
        .waitForNetworkIdle({ idleTime: 700, timeout: NAVIGATION_TIMEOUT_MS })
        .catch(() => {});
      ...
      if (!ok) {
        ...
        onLog?.(
          `캐치 로그인 실패 메시지: ${error || 'ID/비밀번호 오류 또는 접근 차단'}`,
        );
        return {
          ok: false,
          finalUrl,
          failedStep: '로그인 실패',
          error: error || 'ID/비밀번호 오류 또는 접근 차단',
        };
      }
    ...
    try {
      ...
      if (!apiResult.ok) {
        return {
          ...
          error:
            apiResult.message ||
            `캐치 API 로그인 실패 (HTTP ${apiResult.status})`,
        };
      }
      ...
      await page.browserContext().setCookie(
        ...
        {
          name: 'loginPoolPopup',
          value: String(apiResult.loginPoolPopup ?? ''),
          domain: '.catch.co.kr',
          path: '/',
          expires,
        },
        ...(apiResult.refreshToken
          ? [
              {
                name: 'refreshToken',
                value: String(apiResult.refreshToken),
                domain: '.catch.co.kr',
                path: '/',
                expires: Math.floor(
      ...
      return {
        ...
        error: ok
          ? undefined
          : '캐치 API 응답은 성공했지만 로그인 세션을 확인하지 못했습니다.',
      };
    } catch (err) {
  ...
  private async isLoggedIn(page: Page): Promise<boolean> {
    const cookies: Array<{ name: string; value: string }> = await page
      ...
      .cookies()
      .catch(() => []);
    if (cookies.some((cookie) => cookie.name === 'token' && cookie.value))
      return true;
    ...
    return page
      .evaluate(() => {
        ...
        return (
          /로그아웃|마이페이지|MY|지원현황/.test(text) &&
          !/아이디 입력|비밀번호 입력/.test(text)
        );
      })
      .catch(() => false);
  }
    ...
    return page
      .evaluate(() => {
        ...
        const match = text.match(
          /(로그인에 실패했습니다|아이디.*확인|비밀번호.*확인|계정.*잠김|휴대폰번호 인증[^.。]*)/,
        );
        return match?.[0] ?? '';
      })
      .catch(() => '');
