...
export class ImageOcrQueueService implements OnModuleDestroy {
  ...
  enqueue(buffer: Buffer, mimetype: string, filename: string, model = 'gemini-2.0-flash'): string {
    ...
    this.run(jobId, buffer, mimetype, model, subject, abortController.signal).catch((e) => {
      if (!abortController.signal.aborted) {
        subject.next({ data: { type: 'error', message: e instanceof Error ? e.message : '오류' } });
        subject.complete();
      }
