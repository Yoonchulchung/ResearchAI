...
import { BaseJobExecutor } from 'src/queue/application/job/base-job.executor';

const MAX_RETRIES = 2;
const RETRY_DELAY_MS = 1_500;

@Injectable()
export class CompanyAnalysisExecutor extends BaseJobExecutor<
  CompanyAnalysisDto | null,
  [
    companyName: string,
    model: string,
...
> {
  constructor(private readonly companyAnalysisService: CompanyAnalysisService) {
    super(CompanyAnalysisExecutor.name);
  }
