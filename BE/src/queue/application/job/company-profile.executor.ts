...
} from 'src/queue/application/job/base-job.executor';

@Injectable()
export class CompanyProfileExecutor extends StreamingJobExecutor<
  [
    companyName: string,
    model: string,
...
> {
  constructor(
    ...
    super(CompanyProfileExecutor.name);
  }
...
⚠️ 웹 검색을 사용할 수 없어 AI 학습 데이터 기반으로 작성되었습니다. 실제 채용 공고를 반드시 확인하세요.`;
    ...
    return this.collectStream(
      this.aiProvider.stream(model, systemPrompt, [
