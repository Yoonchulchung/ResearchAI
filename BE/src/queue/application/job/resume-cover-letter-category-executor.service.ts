...
export class ResumeCoverLetterCategoryExecutorService {
  private readonly logger = new Logger(ResumeCoverLetterCategoryExecutorService.name);
  ...
  async execute(
    ...
    const items = await this.resumeService.findCoverLettersForCategoryClassification({
    ...
    this.logger.log(`[ResumeCategory] ${items.length} cover letters, model=${model}`);

    const updated: ResumeCoverLetterCategoryResult['updated'] = [];
    for (const [index, item] of items.entries()) {
      if (signal?.aborted) break;
      onLog(`${index + 1}/${items.length} 분류 중: ${this.truncate(item.title || item.answer, 36)}`);
      const category = await this.classifyOne(model, item.title, item.answer, signal);
      ...
      onLog(`${index + 1}/${items.length} 저장 완료: ${category.join(', ') || '기타'}`);
    }
  ...
  private resolveFreeModel(model?: string): string {
    ...
    if (value.startsWith('gemini') || value.startsWith('groq:') || value === 'llama-3.3-70b-versatile') {
      return value;
    }
  ...
  private parseCategories(text: string): string[] {
    ...
    try {
      ...
      if (Array.isArray(parsed.categories)) return parsed.categories.map(String);
    } catch {
    ...
    return COVER_LETTER_CATEGORIES.filter((category) => trimmed.includes(category));
  }
