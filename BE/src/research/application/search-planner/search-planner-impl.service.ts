...
@Injectable()
export class SearchPlannerImplService {
  private readonly logger = new Logger(SearchPlannerImplService.name);

  constructor(private readonly aiProvider: AiProviderService) {}

  async plan(topic: string, localAIModel?: string): Promise<SearchPlan> {
    const CLOUD_PREFIXES = [
      AI_MODEL_PREFIX.ANTHROPIC,
      AI_MODEL_PREFIX.GOOGLE,
      'gpt-',
      'o1',
      'o3',
    ];
    ...
    if (localAIModel !== undefined) {
      ...
      if (trimmed === '') {
        ...
        aiModel = `${AI_MODEL_PREFIX.OLLAMA}${trimmed}`;
      }
    } else {
      const envModel =
...
- 주제에서 신입/경력 등 채용 구분이 명시된 경우만 추출. 없으면 빈 배열 []`;
    ...
    try {
      ...
      return { searchMode, reason: '', keyword, companyTypes, jobTypes, model: aiModel };
    } catch {
