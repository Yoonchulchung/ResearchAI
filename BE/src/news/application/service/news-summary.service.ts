...
export class NewsSummaryService {
  ...
  private async setRawCache(cacheKey: string, data: unknown): Promise<void> {
    ...
    if (existing) {
      ...
      await this.briefingRepo.save({ date: cacheKey, titlesHash: '', summary: '', rawData: JSON.stringify(data) });
    }
  }
  ...
  async getGithubSummary(since: string): Promise<{ summary: string; generatedAt: string; cached: boolean }> {
    ...
    if (!repos) {
      repos = await this.newsProvider.fetchTrendingRepos(validSince as 'daily' | 'weekly' | 'monthly');
  ...
  async getHfSummary(category: string): Promise<{ summary: string; generatedAt: string; cached: boolean }> {
    ...
    if (!items) {
      items = await this.newsProvider.fetchHfTrending(validCategory as 'models' | 'datasets' | 'spaces');
    ...
    if (!titleSample) {
      ...
      for (const r of results) {
        if (r.status === 'fulfilled') {
          allTitles.push(...r.value.map((item: GNewsItem) => item.title));
        }
      }
