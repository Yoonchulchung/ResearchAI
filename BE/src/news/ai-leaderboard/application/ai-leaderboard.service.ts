...
export class AiLeaderboardService implements OnModuleInit, OnModuleDestroy {
  ...
  async getLeaderboard(
    ...
    if (options.type)
      qb = qb.andWhere('e.modelType = :type', { type: options.type });
    if (options.maxParams != null)
      qb = qb.andWhere('e.params <= :maxP', { maxP: options.maxParams });
    if (options.minParams != null)
      qb = qb.andWhere('e.params >= :minP', { minP: options.minParams });
  ...
  private async refreshIfStale(): Promise<void> {
    const counts = await Promise.all(
      ALL_CATEGORIES.map((cat) =>
        this.repo.count({ where: { category: cat } }),
      ),
    );
  ...
  private async fetchAndStoreLlm(): Promise<void> {
    ...
    const mergedBenchmarkEntries = this.mergeEntriesByModel(
      'llm',
      sourceGroups,
    );
  ...
  private async fetchVlmFromDataset(): Promise<NormalizedEntry[]> {
    ...
    for (const split of splits) {
      try {
        const rows = await this.fetchHfDatasetPage(
  ...
  private async fetchVlmFromOpenCompassJson(): Promise<NormalizedEntry[]> {
    ...
    return Object.entries(results)
      .map(([fallbackName, item]): NormalizedEntry | null => {
        ...
        const method = Array.isArray(meta.Method)
          ? meta.Method[0]
          : meta.Method;
        ...
        return {
          ...
          architecture:
            String(
              meta['Language Model'] ?? meta['Vision Model'] ?? '',
            ).trim() || null,
          likes: null,
          benchmarks,
        };
  ...
  private async fetchAndStoreCode(): Promise<void> {
    ...
    if (entries.length === 0) {
      const raw = await this.fetchHfPopularityEntries(
        'text-generation',
        'code',
        150,
        'code',
  ...
  private async fetchCodeFromDataset(): Promise<NormalizedEntry[]> {
    ...
    if (sourceEntries.length > 0)
      return this.mergeEntriesByModel('code', sourceEntries);
  ...
  private async fetchAndStoreTextToImage(): Promise<void> {
    ...
    const artificialAnalysisEntries =
      await this.fetchArtificialAnalysisMediaEntries('text-to-image');
    const hfEntries = await this.fetchHfPopularityEntries(
      'text-to-image',
      'text-to-image',
      120,
    );
    const entries =
      artificialAnalysisEntries.length > 0
        ? this.mergeEntriesByModel('text-to-image', [
  ...
  private async fetchAndStoreImageEditing(): Promise<void> {
    ...
    const entries =
      await this.fetchArtificialAnalysisMediaEntries('image-editing');
  ...
  private async fetchAndStoreTextToVideo(): Promise<void> {
    ...
    const entries =
      await this.fetchArtificialAnalysisMediaEntries('text-to-video');
  ...
  private async fetchAndStoreImageToVideo(): Promise<void> {
    ...
    const entries =
      await this.fetchArtificialAnalysisMediaEntries('image-to-video');
    await this.storeEntries(entries, 'image-to-video');
    this.logger.log(`Stored ${entries.length} Image-to-Video entries`);
  }

  private async fetchArtificialAnalysisMediaEntries(
    category: string,
    ...
    ) as ArtificialMediaModel[];
    const eloValues = rows
      .map((row) => nAny(row.elo))
      .filter((value): value is number => value != null);
    const minElo = Math.min(...eloValues);
    const maxElo = Math.max(...eloValues);

    return rows
      .map((row): NormalizedEntry | null => {
        const fullname = String(row.name ?? row.slug ?? row.id ?? '').trim();
        if (!fullname) return null;
        const creator = row.model_creator ?? {};
        const org = String(creator.name ?? creator.slug ?? '').trim();
  ...
  private async fetchHfPopularityEntries(
    ...
    try {
      const models = (await this.hfFetch(url)) as Array<{
        id: string;
        likes?: number;
        downloads?: number;
        author?: string;
      ...
      const maxLikeScore = Math.max(
        ...likeValues.map((value) => Math.log1p(value)),
        1,
      );
      const maxDownloadScore = Math.max(
        ...downloadValues.map((value) => Math.log1p(value)),
        1,
      );
      ...
      return rows.map((m): NormalizedEntry => {
        ...
        const likeScore =
          likes != null
            ? Math.round((Math.log1p(likes) / maxLikeScore) * 1000) / 10
            : null;
        const downloadScore =
          downloads != null
            ? Math.round((Math.log1p(downloads) / maxDownloadScore) * 1000) / 10
            : null;
      ...
      this.logger.warn(
        `HF models API failed for ${pipelineTag}: ${e instanceof Error ? e.message : e}`,
      );
      return [];
    }
  }
  ...
  private async addHfPopularityEvidence(
    ...
    const popularityEntries = await this.fetchHfPopularityEntries(
      pipelineTag,
      category,
      limit,
      tagFilter,
    ...
    if (benchmarkEntries.length === 0)
      return this.rankEntries(popularityEntries);
    const benchmarkKeys = new Set(
      benchmarkEntries.map((entry) => this.modelKey(entry.fullname)),
    );
    return this.mergeEntriesByModel(category, [
      benchmarkEntries,
      popularityEntries,
  ...
  private parseParamB(value: unknown): number | null {
    ...
    if (/m\b/i.test(text) && !/b\b/i.test(text))
      return Math.round((numeric / 1000) * 1000) / 1000;
    return numeric;
  }

  private withSourceScores(
  ...
  private mergeEntriesByModel(
    category: string,
    sourceEntryGroups: NormalizedEntry[][],
  ): NormalizedEntry[] {
    ...
    for (const entries of sourceEntryGroups) {
      for (const entry of entries) {
        ...
        for (const [benchmark, value] of Object.entries(
          entry.benchmarks ?? {},
        )) {
          if (mergedBenchmarks[benchmark] == null && value != null)
            mergedBenchmarks[benchmark] = value;
        }
  ...
  private toEntry(e: AiLeaderboardEntryEntity): AiModelEntry {
    ...
    try {
      benchmarks = JSON.parse(e.benchmarksJson || '{}');
    } catch {
      /* noop */
    }
    try {
      sourceScores = JSON.parse(e.sourceScoresJson || '{}');
    } catch {
      /* noop */
    }
