...
function vectorCosineSimilarity(
...
export function filterDuplicatePostingsByDeadlineAndTitle<
  T extends DeadlineTitlePosting,
>(postings: T[], threshold = DEFAULT_TITLE_COSINE_THRESHOLD): T[] {
  ...
  const uniquePostings: T[] = [...withoutDeadline];

  for (const group of groups.values()) {
    if (group.length === 1) {
      uniquePostings.push(group[0]);
      continue;
    }

    const parent = group.map((_, index) => index);
    const normalizedTitles = group.map((posting) =>
    ...
    for (let left = 0; left < group.length; left++) {
      for (let right = left + 1; right < group.length; right++) {
        const similarity =
          normalizedTitles[left] &&
          normalizedTitles[left] === normalizedTitles[right]
            ? 1
    ...
    for (const cluster of clusters.values())
      uniquePostings.push(preferredPosting(cluster));
  }
