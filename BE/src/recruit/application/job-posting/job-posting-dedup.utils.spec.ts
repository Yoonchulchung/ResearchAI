import {
  deduplicatePostingsByDeadlineAndTitle,
  getPostingDeadlineKey,
  titleCosineSimilarity,
} from './job-posting-dedup.utils';

describe('job posting deadline/title deduplication', () => {
  ...
  it('removes similar titles only when the deadline key matches', () => {
    ...
    const result = deduplicatePostingsByDeadlineAndTitle(items);
