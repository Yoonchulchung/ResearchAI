...
export class CoverLetterScrapeEngineService implements OnModuleInit, OnModuleDestroy {
  ...
  async startScraping(opts: ScrapeOptions = {}): Promise<{ message: string }> {
    ...
    const runOpts: InternalScrapeOptions = {
      ...opts,
      catchCredentials:
        source === 'catch' || source === 'all' ? getCatchCredentials() : undefined,
    };
  ...
  async backfillQuestionRows(): Promise<{ updated: number }> {
    ...
    for (let i = 0; i < rows.length; i += chunkSize) {
      ...
      await this.questionRepo.delete({ coverLetterId: In(chunk.map((r) => r.id)) });
  ...
  private async runScraping(opts: InternalScrapeOptions) {
    ...
    try {
      if (source === 'all') {
        ...
        await this.runSourceLoop(source as 'linkareer' | 'catch', opts);
      }
    } finally {
      this.status.running = false;
  ...
  private toEntityData(item: CoverLetter) {
    ...
    return this.coverLetterRepo.create({
      ...
      jobCategory: normalized.jobCategory ?? inferJobCategory(normalized.position),
  ...
  private toQuestionEntities(item: CoverLetter) {
    return (item.questions ?? []).map((question, index) => {
      ...
      return this.questionRepo.create({
        ...
        searchText: (
          [item.company, item.position, item.season, item.spec, question.question, question.answer, ...tags, ...keywords]
            .filter(Boolean)
            .join('\n')
