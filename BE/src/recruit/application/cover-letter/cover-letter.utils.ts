...
export function inferJobCategory(position: string): JobCategory {
  ...
  if (
    /반도체|semiconductor|회로|하드웨어|hw|임베디드|embedded|펌웨어|firmware|디스플레이|display|전기전자|전자공학|제어공학|rf[엔공]|fpga|pcb|vlsi|fab공정|웨이퍼|패키지공정|메모리설계|아날로그|시스템반도체|파운드리|eda|소자/.test(
      p,
    )
  ) {
    return '전자';
  ...
  if (
    /백엔드|프론트엔드|풀스택|fullstack|앱개발|모바일개발|소프트웨어|software|개발자|sw엔지니어|웹개발|데이터엔지니어|데이터분析|데이터사이언|dataengineer|datascienc|ai[엔개]|머신러닝|machinelearn|딥러닝|deeplearn|클라우드|cloud|보안|security|인프라|infra|서버|server|네트워크|network|sre|devops|dba|qa|정보보안|정보기술|it[엔개서]|플랫폼|platform|si[개업]|사이버|cyber|blockchain|블록체인/.test(
      p,
    )
  ) {
    return 'IT';
  ...
  if (
    /영업|세일즈|sales|거래처|b2b|b2c|고객관리|채널영업|솔루션영업|기술영업|대리점/.test(
      p,
    )
  )
    return '영업';
  if (
    /마케팅|marketing|광고|홍보|pr[팀담]|브랜드|brand|sns|콘텐츠|content|퍼포먼스|디지털마케팅|crm|그로스/.test(
      p,
    )
  )
    return '마케팅';
  if (
    /재무|회계|accounting|세무|tax|자금|원가|financial|audit|감사|fp&a|cfr|irm/.test(
      p,
    )
  )
    return '재무/회계';
  if (/인사|hr[팀담]|채용|인재|조직문화|노무|총무|hrd|교육훈련/.test(p))
    return '인사/총무';
  if (
    /생산관리|생산기술|품질관리|공정관리|scm|물류|구매|설비|manufacturing|공장|제조|qc[팀담]|qm/.test(
      p,
    )
  )
    return '생산/제조';
  if (
    /기획|전략|경영|컨설팅|consulting|사업개발|bizdev|프로젝트매니저|pm[^a-z]|사업기획|신사업|전략기획/.test(
      p,
    )
  )
    return '경영/기획';
...
export function getCatchCredentials():
...
export function classifyQuestionTags(
  ...
  return classifyQuestionTitle(question.question ?? '', question.answer ?? '');
}
...
export function normalizeCoverLetterForView(item: CoverLetter): CoverLetter {
  return {
    ...item,
    source:
      item.source ?? (item.id.startsWith('catch-') ? 'catch' : 'linkareer'),
...
export function parseLegacyQuestions(
  value?: string | null,
): CoverLetterQuestion[] {
  try {
    const parsed = JSON.parse(value || '[]');
    ...
    return parsed.map((question, index) => {
      ...
      return {
        ...normalized,
        tags,
        keywords: extractQuestionKeywords(normalized, tags),
      };
    });
  } catch {
    return [];
...
export function toCoverLetter(
  ...
  const questions =
    relationQuestions.length > 0
      ? relationQuestions
      : parseLegacyQuestions(entity.questions);
...
export function entityToJobAnalysis(
  row: CoverLetterSpecAnalysisEntity,
): CoverLetterJobAnalysis {
  let extractedSpec: CoverLetterJobAnalysis['extractedSpec'];
  if (
    (row.school !== undefined && row.school !== null) ||
    (row.specSummary !== undefined && row.specSummary !== null)
  ) {
    extractedSpec = {
      school: row.school || '',
      major: row.major || '',
