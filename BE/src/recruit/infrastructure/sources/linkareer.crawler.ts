      ...
      while (collected < limit && pageNum <= 3) {
        ...
        const items = await page.evaluate((baseUrl: string) => {
          ...
          for (const link of links) {
            ...
            const card =
              link.closest(
                'li, article, [class*="Card"], [class*="card"], [class*="Item"], [class*="item"]',
              ) ?? link.parentElement;
            if (!card) continue;
            ...
            const textNodes = Array.from(card.querySelectorAll('span,p,div'))
              ...
              .filter((t) => t && t !== title);
            const company =
              textNodes.find((t) => t.length > 0 && t.length < 50) ?? '';

            // 태그: [태그] 패턴 또는 작은 span들
            const tagEls = Array.from(
              card.querySelectorAll(
                'span[class*="tag"],span[class*="Tag"],span[class*="badge"],span[class*="Badge"]',
              ),
            );
            const tags = tagEls
        ...
        for (const item of items) {
          ...
          if (
            !isJobRelated(combined) &&
            query.keyword &&
            !combined.includes(query.keyword)
          )
            continue;
