...
export class MultiSiteJobCrawler implements JobSource {
  ...
  async *collect(query: CollectQuery): AsyncGenerator<JobPosting> {
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(`[링커리어] 실패 — ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      try {
        ...
        this.logger.warn(`[원티드] 실패 — ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      try {
        ...
        this.logger.warn(`[잡플래닛] 실패 — ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      for (const job of allJobs) {
        yield {
          ...
          skills: job.tags ? job.tags.split(',').map((t) => t.trim()).filter(Boolean) : [],
          url: job.url,
  ...
  private async scrapeLinkareer(page: Page, keyword: string, limit: number, jobTypes?: string[]): Promise<RawJob[]> {
    ...
    return page.evaluate((baseUrl: string, lim: number, jt: string[] | undefined) => {
      ...
      const cards = Array.from(document.querySelectorAll<HTMLElement>('[class*="ActivityListItem"], [class*="activity-list-item"], li, article'));
      for (const card of cards) {
        if (results.length >= lim) break;

        // 채용 칩만 허용 (대외활동, 공모전 등 제외)
        const chipLabel = card.querySelector('.MuiChip-label, [class*="label-chip"]')?.textContent?.trim() ?? '';
        ...
        const title = (card.querySelector('p.title, [class*="title"]')?.textContent ?? link.textContent ?? '').replace(/\s+/g, ' ').trim();
        if (!title) continue;

        const company = (card.querySelector('p.company-name, [class*="company"]')?.textContent ?? '').replace(/\s+/g, ' ').trim();

        // 신입/경력 필터
        const jobTypeText = Array.from(card.querySelectorAll('p.short-info-typo, [class*="short-info"]'))
          .map((el) => el.textContent?.trim() ?? '')
          .join(' ');
        if (jt && jt.length > 0) {
          const matched = jt.some((t) => jobTypeText.includes(t) || (t === '신입' && jobTypeText.includes('신입')) || (t === '경력' && jobTypeText.includes('경력')));
          if (!matched) continue;
        }

        const text = card.textContent ?? '';
        const deadlineMatch = text.match(/~\d+\.\d+|\d{4}[./]\d{2}[./]\d{2}|D-\d+/);
      ...
      return results;
    }, 'https://linkareer.com', limit, jobTypes);
  }

  // ── 원티드 ──────────────────────────────────────────────────────────────────
  private async scrapeWanted(page: Page, keyword: string, limit: number): Promise<RawJob[]> {
    ...
    return page.evaluate((baseUrl: string, lim: number) => {
      const results: { title: string; company: string; tags: string; deadline: string; url: string; source: string }[] = [];
      const seen = new Set<string>();

      // /wd/{id} 패턴이 원티드 포지션 링크
      const links = Array.from(document.querySelectorAll<HTMLAnchorElement>('a[href*="/wd/"]'));
      for (const link of links) {
        ...
        const titleEl = link.querySelector('strong, h2, h3, [class*="title"], [class*="Title"]') ?? link;
        const title = (titleEl.textContent ?? '').replace(/\s+/g, ' ').trim();
        if (!title) continue;

        const spans = Array.from(card.querySelectorAll('span')).map((el) => (el.textContent ?? '').replace(/\s+/g, ' ').trim()).filter((t) => t && t !== title && t.length < 50);
        const company = spans[0] ?? '';

        results.push({ title, company, tags: '', deadline: '', url: href.startsWith('http') ? href : baseUrl + href, source: 'wanted' });
      }
      return results;
    }, 'https://www.wanted.co.kr', limit);
  }

  // ── 잡플래닛 채용 ────────────────────────────────────────────────────────────
  private async scrapeJobplanet(page: Page, keyword: string, limit: number, jobTypes?: string[]): Promise<RawJob[]> {
    ...
    return page.evaluate((lim: number, jt: string[] | undefined) => {
      ...
      try {
        const nextData = JSON.parse(nextDataEl.textContent) as Record<string, unknown>;
        ...
        const match = raw.match(/"job_postings?":\s*(\[[\s\S]*?\])|"postings?":\s*(\[[\s\S]*?\])|"list":\s*(\[[\s\S]*?\])/);
        ...
        const findPostings = (obj: unknown): unknown[] => {
          ...
          if (Array.isArray(obj)) {
            ...
            if (obj.length > 0 && typeof obj[0] === 'object' && obj[0] !== null &&
                'title' in (obj[0] as object) && 'company' in (obj[0] as object)) {
              return obj;
            }
        ...
        if (postings.length === 0 && match) {
          // 정규식 fallback
          postings = JSON.parse(match[1] ?? match[2] ?? match[3] ?? '[]') as unknown[];
        }
        ...
        for (const p of postings) {
          ...
          const skills = Array.isArray(post.skills) ? (post.skills as string[]).join(', ') : '';
          const recruitmentText = Array.isArray(post.recruitment_text)
            ? (post.recruitment_text as string[]).join(', ')
            : (post.annual as Record<string, unknown> | undefined)?.text as string ?? '';
          const jobType = (post.job_type as string | undefined) ?? '';
          const deadline = (post.deadline_message as string | undefined) ?? (post.end_at as string | undefined) ?? '';

          // jobTypes 필터
          if (jt && jt.length > 0) {
            const matched = jt.some((t) => recruitmentText.includes(t) || jobType.includes(t));
            if (!matched) continue;
          }

          const tags = [recruitmentText, jobType, cityName, skills].filter(Boolean).join(', ');
  ...
  // ── 인크루트 ────────────────────────────────────────────────────────────────
  private async scrapeIncruit(page: Page, keyword: string, limit: number, jobTypes?: string[]): Promise<RawJob[]> {
    ...
    return page.evaluate((lim: number, jt: string[] | undefined) => {
      ...
      for (const row of rows) {
        ...
        const metaSpans = Array.from(row.querySelectorAll('.cl_md span')).map((el) => el.textContent?.trim() ?? '').filter(Boolean);
        const tags = metaSpans.join(', ');

        // 신입/경력 필터
        if (jt && jt.length > 0) {
          const matched = jt.some((t) => metaSpans.some((s) => s.includes(t)));
          if (!matched) continue;
        }
