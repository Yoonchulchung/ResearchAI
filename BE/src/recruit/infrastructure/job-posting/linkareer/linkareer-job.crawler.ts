...
import { JobPostingCrawlerPort } from 'src/recruit/application/job-posting/ports/job-posting-crawler.port';
import type {
  JobPostingDetail,
...
@Injectable()
export class LinkareerJobCrawler extends JobPostingCrawlerPort {
  readonly source = 'linkareer' as const;

  /** GraphQL API로 목록 조회 */
  async getPostingsFromPage(
    request: JobPostingPageRequest,
  ): Promise<JobPosting[]> {
    const { page, jobType = 'INTERN', status } = request;
    const isIntern = jobType === 'INTERN';
    const variables = {
      filterBy: {
        status: status === 'ALL' ? null : 'OPEN',
        activityTypeID: '5',
        regionIDs: [],
  ...
  async getDetail(request: JobPostingDetailRequest): Promise<JobPostingDetail> {
    const { id } = request;
