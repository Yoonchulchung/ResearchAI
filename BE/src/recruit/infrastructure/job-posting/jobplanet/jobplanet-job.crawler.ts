...
import type { JobPostingPageRequest } from 'src/recruit/application/job-posting/job-posting-crawler.types';
...
@Injectable()
export class JobplanetJobCrawler extends JobPostingCrawlerPort {
  readonly source = 'jobplanet' as const;

  async getPostingsFromPage(
    request: JobPostingPageRequest,
  ): Promise<JobPosting[]> {
    const { page, pageSize = 8 } = request;
