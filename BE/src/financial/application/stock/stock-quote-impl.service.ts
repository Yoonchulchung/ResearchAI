...
@Injectable()
export class StockQuoteImplService {
  constructor(
        ...
        if (!before && naverStockData?.regularMarketPrice != null) {
          ...
          if (interval === '15m' && naverStockData.tradingDate) {
            ...
            if (todayChart.length > 0) {
              quote.chart = [
                ...quote.chart.filter(
                  (c) => !c.date.startsWith(naverStockData.tradingDate!),
                ),
                ...todayChart,
  ...
  private async fetchYahooChart(
    ...
    type Candle = StockQuote['chart'][number];
        ...
        ? raw.reduce<Candle[]>((acc, candle, i) => {
            if (i % aggregate === 0) {
              ...
              acc.push({
                date: candle.date,
                open: candle.open,
                high: hs.length ? Math.max(...hs) : null,
                low: ls.length ? Math.min(...ls) : null,
  ...
  private async fetchNaverStockData(stockCode: string): Promise<{
    ...
    marketMetrics: NonNullable<StockQuote['marketMetrics']>;
  } | null> {
  ...
  private async fetchNaverMinuteChart(
    ...
    const grouped = new Map<string, StockQuote['chart'][number]>();
