...
import {
  KrxInvestorService,
  InvestorTradingData,
} from 'src/financial/infrastructure/krx/krx-investor.service';

@Injectable()
export class FinancialInvestorTradingImplService {
  constructor(
  ...
  async getBySymbol(symbol: string, days = 30): Promise<InvestorTradingData> {
  ...
  private async fetchAndCache(
    ...
    const normalizedDays = Math.min(Math.max(Number(days) || 30, 1), 120);
    const cacheKey = `investor-trading:${stockCode}:${normalizedDays}`;
    const today = this.todayKey();

    const cached = await this.dbGet<
      InvestorTradingData & { fetchedDate: string }
    >(cacheKey);
    if (
      cached &&
