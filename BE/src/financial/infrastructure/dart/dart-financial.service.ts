...
export class DartFinancialService {
  ...
  async fetchMultiYearFinancials(
    ...
    for (const settled of responses) {
      ...
      const nonCurrentAssetsRaw =
        find(['비유동자산'], 'BS') ?? find(['비유동자산']);
      const tangibleAssetsRaw = find(['유형자산'], 'BS') ?? find(['유형자산']);
      const intangibleAssetsRaw =
        find(['무형자산'], 'BS') ?? find(['무형자산']);
      const totalLiabilitiesRaw =
        find(['부채총계'], 'BS') ?? find(['부채총계']);
      const nonCurrentLiabRaw =
        find(['비유동부채'], 'BS') ?? find(['비유동부채']);
      ...
      const receivablesRaw =
        find(
          ['매출채권및기타채권', '매출채권', '매출채권 및 기타채권'],
          'BS',
        ) ?? find(['매출채권및기타채권', '매출채권', '매출채권 및 기타채권']);
      const currentLiabRaw = find(['유동부채'], 'BS') ?? find(['유동부채']);
      const shortTermBorrowRaw =
        find(['단기차입금'], 'BS') ?? find(['단기차입금']);
      const longTermBorrowRaw =
        find(['장기차입금'], 'BS') ?? find(['장기차입금']);
      ...
      const totalBorrowings = add(
        add(shortTermBorrowings, longTermBorrowings),
        bonds,
      );
      const netDebt =
  ...
  async fetchRecentQuarterlyFinancials(
    ...
    for (const result of settled) {
      ...
      const currentLiabilitiesRaw =
        find(['유동부채'], 'BS') ?? find(['유동부채']);
      const shortTermBorrowRaw =
        find(['단기차입금'], 'BS') ?? find(['단기차입금']);
      const longTermBorrowRaw =
        find(['장기차입금'], 'BS') ?? find(['장기차입금']);
      ...
      const addNull = (a: number | null, b: number | null) =>
        a != null || b != null ? (a ?? 0) + (b ?? 0) : null;
      const totalBorrowings = addNull(
        addNull(shortTermBorrowings, longTermBorrowings),
        bonds,
      );
      const netDebt =
        totalBorrowings != null && cashAndEquivalents != null
      ...
      const operatingCashFlow = toEok(
        find(CF_NAMES.op, 'CF') ?? find(CF_NAMES.op),
      );
      const investingCashFlow = toEok(
        find(CF_NAMES.inv, 'CF') ?? find(CF_NAMES.inv),
      );
      const financingCashFlow = toEok(
        find(CF_NAMES.fin, 'CF') ?? find(CF_NAMES.fin),
      );
      ...
      const item: CumulativeQuarter = {
        ...
        interestCoverageRatio:
          ...
          interestExpense !== 0
            ? Math.round((operatingProfit / interestExpense) * 100) / 100
            : null,
        totalAssets,
        nonCurrentAssets,
        tangibleAssets,
        ...
        debtRatio: pct(toWon(totalLiabilitiesRaw), toWon(totalEquityRaw)),
        currentRatio: pct(
          toWon(currentAssetsRaw),
          toWon(currentLiabilitiesRaw),
        ),
        netDebtRatio: pct(netDebt, totalEquity),
        operatingCashFlow,
        investingCashFlow,
