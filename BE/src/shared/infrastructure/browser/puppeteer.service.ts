...
import { BROWSER_ARGS, BROWSER_USER_AGENT } from 'src/shared/infrastructure/browser/browser-automation.util';
...
export class PuppeteerService implements OnModuleInit, OnModuleDestroy {
  ...
  async searchGoogle(
    ...
    try {
      ...
      this.logger.log(
        `searchGoogle(DDG): query="${query}" results=${results.length}`,
      );
      return results;
    } finally {
      await page.close();
