...
function useSummary(path: string) {
  ...
  const fetch_ = useCallback(() => {
    setLoading(true);
    apiFetch<{ summary: string; aiModel?: string | null }>(path)
      .then((data) => {
  ...
  return { summary, aiModel, loading, lastUpdated, attempted, refresh: fetch_ };
}
    ...
    <div className="glass-panel rounded-2xl p-5">
      ...
      {!active.attempted || active.loading ? (
        ...
        <>
          ...
          {active.aiModel && (
            <p className="mt-3 text-xs text-slate-400 text-right">generated by {active.aiModel}</p>
          )}
        </>
      ) : (
