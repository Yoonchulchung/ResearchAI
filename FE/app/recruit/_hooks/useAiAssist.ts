...
function chatKey(docId: string | null) {
  return `recruit-chat:${docId ?? "new"}`;
}

function pendingJobKey(docId: string | null) {
  return `recruit-pending:${docId ?? "new"}`;
}
  ...
  const runAssist = async (
    ...
    try {
      ...
      if (actionKey) {
        ...
        ({ jobId } = await enqueueRecruitAssist(actionKey, targetContent, model, experiences, instruction || undefined));
      } else {
        const expContext =
