...
function TypeBadge({ type }: { type: ResumeSearchItem["type"] }) {
  const map = {
    ...
    experience: { label: "경험", cls: "bg-emerald-50 text-emerald-600" },
  ...
  return (
    <div className="rounded-xl border border-slate-100 bg-white p-3 flex flex-col gap-2">
      <div className="flex items-start justify-between gap-2">
        ...
        {onInsert && (
          <button
            onClick={() => onInsert({ id: uid(), question: item.question, answer: item.answer })}
            className="shrink-0 flex items-center gap-1 rounded-lg border border-indigo-200 px-2 py-1 text-[11px] font-bold text-indigo-600 hover:bg-indigo-50 transition-colors"
          >
      ...
      {item.answer && (
        <>
          <p className={`text-xs text-slate-500 leading-relaxed ${open ? "" : "line-clamp-2"}`}>
            {item.answer}
          </p>
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {onInsert && (
          <button
            ...
            className="shrink-0 flex items-center gap-1 rounded-lg border border-emerald-200 px-2 py-1 text-[11px] font-bold text-emerald-600 hover:bg-emerald-50 transition-colors"
          >
            <svg width="9" height="9" viewBox="0 0 9 9" fill="none"><path d="M4.5 1v7M1 4.5h7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
            추가
          </button>
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {onInsert && (
          <button
            ...
            className="shrink-0 flex items-center gap-1 rounded-lg border border-amber-200 px-2 py-1 text-[11px] font-bold text-amber-600 hover:bg-amber-50 transition-colors"
          >
            <svg width="9" height="9" viewBox="0 0 9 9" fill="none"><path d="M4.5 1v7M1 4.5h7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
            추가
          </button>
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50/60">
      ...
      <div className="shrink-0 px-4 pt-5 pb-3 border-b border-slate-100">
        <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400 mb-3">경험 검색</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-4">
        {!query.trim() ? (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
            ...
            <p className="text-xs text-slate-400">이전 이력서의 자소서·경험·수상을<br />검색해서 현재 항목에 추가할 수 있어요.</p>
          </div>
          ...
          <>
            ...
            {experiences.length > 0 && (
              <div className="flex flex-col gap-2">
                <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">경험 ({experiences.length})</p>
