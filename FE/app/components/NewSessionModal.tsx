...
function ModalContent({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* ── Footer ─────────────────────────────────────────────────────────── */}
      <div className="shrink-0 flex items-center gap-3 px-4 sm:px-6 py-4 border-t border-slate-100 bg-white">
        <div className="flex-1 text-xs text-slate-400 truncate min-w-0">
          {tasks.length > 0
            ? searchSource === "recruit"
              ? "채용 공고 검색"
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-slate-600 rounded-lg hover:bg-slate-100 transition-colors whitespace-nowrap"
          >
            취소
          </button>
          <button
            ...
            className="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-sm shadow-indigo-100 whitespace-nowrap"
          >
            {creating ? (
