  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-[#F4F5F7]">
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        {/* Topbar */}
        <div className="shrink-0 flex items-center gap-3 px-6 py-3 bg-white border-b border-slate-200/80 shadow-sm z-10">
          <button
            onClick={() => router.back()}
          ...
          <span className="text-base font-bold text-slate-800 tracking-tight">채용 공고</span>
          <span className="px-2 py-0.5 rounded-full bg-slate-100 text-xs font-semibold text-slate-500">{total.toLocaleString()}건</span>
          <div className="flex-1" />

          {status?.running && (
            <div className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-md bg-emerald-50 text-emerald-600 font-medium border border-emerald-100">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              수집 중 {status.totalCollected.toLocaleString()}건 · p.{status.currentPage}
            </div>
          )}

          {!status?.running && scrapeSource === "linkareer" && (
            <div className="flex rounded-md border border-slate-200 overflow-hidden text-xs font-semibold bg-white">
          ...
          {status?.running ? (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold rounded-md border transition-all disabled:opacity-50 bg-white text-red-600 border-red-200 hover:bg-red-50"
            >
            ...
            <button
              onClick={handleStart}
              disabled={scrapeLoading}
              className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold rounded-md border transition-all disabled:opacity-50 bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700 shadow-sm"
            >
              {scrapeLoading ? (
