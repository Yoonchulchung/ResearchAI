  ...
  return (
    <article className={`flex h-full flex-col rounded-xl border p-4 shadow-sm transition ${read ? "opacity-65" : ""} ${isDark ? read ? "border-white/5 bg-white/[0.025] hover:border-indigo-400/20" : "border-white/10 bg-white/5 hover:border-indigo-400/30" : read ? "border-slate-200 bg-slate-50 hover:border-indigo-100" : "border-slate-200 bg-white hover:border-indigo-200 hover:shadow-md"}`}>
      ...
      <div className="mt-auto flex flex-wrap justify-end gap-2 pt-4">
        ...
        {paper.codeUrl && <a href={paper.codeUrl} target="_blank" rel="noreferrer" onClick={() => onMarkRead(paper)} className={`rounded-lg border px-2.5 py-1.5 text-xs font-semibold ${isDark ? "border-white/10 text-white/60 hover:bg-white/5" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>Code</a>}
        <a href={paper.url} target="_blank" rel="noreferrer" onClick={() => onMarkRead(paper)} className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold ${isDark ? "bg-white/10 text-white hover:bg-indigo-500/30" : "bg-slate-900 text-white hover:bg-indigo-600"}`}>원문 보기</a>
      </div>
    </article>
  );
}
